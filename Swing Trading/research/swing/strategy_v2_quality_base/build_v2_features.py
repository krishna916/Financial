"""Build Strategy V2 adjusted price features and point-in-time RS inputs."""
...
def compute_price_features(frame: pd.DataFrame) -> pd.DataFrame:
    """Add locked V2 indicators to one adjusted OHLCV frame."""
    ...
    all_dates = sorted(
        {
            ...
            for date in frame.loc[
                frame["Date"].between(SIGNAL_START, SIGNAL_END), "Date"
            ].dropna()
        }
    )
...
def data_quality_audit(
    ...
    if errors:
        # Keep the required compact schema; callers print errors without hiding them.
...
def build_feature_frames(
    ...
    errors: dict[str, str] = {}
    pairs = (
        membership.loc[membership["Downloadable"] & membership["Yahoo_Ticker"].ne(""), ["Symbol", "Yahoo_Ticker"]]
        .drop_duplicates()
        .sort_values("Symbol")
    )
